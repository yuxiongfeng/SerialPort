...
public class TerminalFragment extends Fragment implements SerialInputOutputManager.Listener {
     ...
     * 发送一次指令的所有data[]
     */
    private byte[] oneDatas = new byte[8192];
    /**
     * 用于标识是否点击了新指令
     */
...
/*    @Override
    public void onResume() {
        super.onResume();
        etActivity().registerReceiver(broadcastReceiver, new IntentFilter(INTENT_ACTION_GRANT_USB));
    ...
    private void connect() {
        ...
        UsbSerialDriver driver = UsbSerialProber.getDefaultProber(getContext()).probeDevice(device);
        if (driver == null) {
    ...
    class ControlLines {
        ...
        private boolean refresh() {
            ...
            try {
                ctrl = "RTS";
                rtsBtn.setChecked(usbSerialPort.getRTS());
                ctrl = "CTS";
                ctsBtn.setChecked(usbSerialPort.getCTS());
                ctrl = "DTR";
                dtrBtn.setChecked(usbSerialPort.getDTR());
                ctrl = "DSR";
                dsrBtn.setChecked(usbSerialPort.getDSR());
                ctrl = "CD";
                cdBtn.setChecked(usbSerialPort.getCD());
                ctrl = "RI";
                riBtn.setChecked(usbSerialPort.getRI());
            } catch (IOException e) {
